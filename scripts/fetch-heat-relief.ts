 ...
 *
 * Handling both is the point. A "multi-city" product that only works where one
 * particular agency publishes one particular schema is not multi-city, and the
 * mapping below is where that claim is either honoured or quietly broken.
 *
...
async function fetchRegion(region: Region) {
  ...
  try {
    ...
    if (features.length === 0) throw new Error('service returned zero features');
  } catch (err) {
